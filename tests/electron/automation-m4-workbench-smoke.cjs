...
app.whenReady().then(async () => {
  ...
  const editorRoundTrip = await win.webContents.executeJavaScript(`(() => { const workflow = JSON.parse(document.querySelector('.automation-json-editor textarea').value); return { ready: workflow.readyWhen, condition: workflow.root.steps.find(item => item.type === 'if-condition')?.condition, wait: workflow.root.steps.find(item => item.type === 'wait-image'), click: workflow.root.steps.find(item => item.type === 'click-image'), coordinateClick: workflow.root.steps.find(item => item.type === 'click-coordinate'), randomClick: workflow.root.steps.find(item => item.type === 'random-click-region'), moveCoordinate: workflow.root.steps.find(item => item.type === 'move-to-coordinate'), generalDrag: workflow.root.steps.find(item => item.type === 'drag'), waitBranch: workflow.root.steps.find(item => item.type === 'wait-condition-branch'), drag: workflow.root.steps.find(item => item.type === 'drag-image') }; })()`);
