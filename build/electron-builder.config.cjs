...
if (bundleOcr && !hasAutomation) {
  throw new Error('BAO_OCR_BUNDLE=1 requires the automation module');
}
...
function selectedResources() {
  if (targetPlatform === 'win32' && targetArch === 'x64') {
    ...
    if (hasDownload) resources.push({ from: 'native/aria2/aria2c.exe', to: 'native/aria2/aria2c.exe' });
  ...
  if (targetPlatform === 'win32' && targetArch === 'ia32') {
    ...
    if (hasDownload) resources.push({ from: 'native/aria2/win32/aria2c.exe', to: 'native/aria2/aria2c.exe' });
    return resources;
  }
  ...
  if (targetPlatform === 'linux' && targetArch === 'x64') {
    ...
    if (hasDownload) resources.push({ from: 'native/aria2/aria2c', to: 'native/aria2/aria2c' });
...
module.exports = {
  ...
  files: [
    // Keep at least one positive root-relative glob before any exclusion.
    // A list made only of FileSet objects plus a negative string makes
    // electron-builder inject its implicit `**/*`, packaging the workspace
    // (including .cache/release) into app.asar.
    'dist/**/*',
    {
    ...
    ...(!hasAutomation ? ['!node_modules/@techstark/opencv-js{,/**/*}'] : []),
  ],
  ...
  asarUnpack: hasAutomation ? [
    'dist/vision-worker.cjs',
    'dist/color-vision-worker.cjs',
  ...
  beforePack: async () => {
    ...
    const executableFiles = ['native/mouse-hook-linux'];
    if (hasDownload) executableFiles.push('native/aria2/aria2c');
