...
export const AUTOMATION_V2_BLOCK_TYPES = Object.freeze([
  ...
  'bao2_call_script',
  'bao2_if', 'bao2_repeat', 'bao2_while', 'bao2_break', 'bao2_continue', 'bao2_wait', 'bao2_wait_target',
...
const labels = {
  'zh-CN': {
    ...
    if: '如果 %1 那么 %2 否则 %3', repeat: '重复 %1 次 %2', while: '当 %1 时循环 %2', break: '跳出循环', continue: '继续下一轮', wait: '等待(ms) %1', waitTarget: '等待 %1 %2 超时(ms) %3',
  ...
  en: {
    ...
    if: 'if %1 then %2 else %3', repeat: 'repeat %1 times %2', while: 'while %1 %2', break: 'break loop', continue: 'continue loop', wait: 'wait(ms) %1', waitTarget: 'wait until %1 is %2 timeout(ms) %3',
...
export function automationV2Toolbox(locale: Locale): Blockly.utils.toolbox.ToolboxDefinition {
  ...
  return { kind: 'categoryToolbox', contents: [
    ...
    { kind: 'category', name: l.catControl, colour: '45', contents: [valuePreset('bao2_repeat', 'COUNT', 'bao2_literal_number'), valuePreset('bao2_wait', 'DURATION', 'bao2_literal_number'), ...blocks(['bao2_if', 'bao2_while', 'bao2_break', 'bao2_continue'])] },
