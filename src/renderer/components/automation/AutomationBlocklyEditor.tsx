...
import type { AutomationCondition, AutomationStep, AutomationWorkflow, ImageCondition, SequenceStep } from '@shared/automation/types';
...
function assetField(assets: string[], fallback: string): { type: string; name: string; options: string[][] } {
  ...
  return { type: 'field_dropdown', name: 'ASSET', options: values.map((value) => [value, value]) };
}
...
function buildBlockDefinitions(LL: ReturnType<typeof useI18nContext>['LL'], assets: string[]): Array<{ type: string; [key: string]: unknown }> {
  ...
  return [
    ...
    { type: 'bao_start', message0: b.start(), args0: [assetField(assets, 'ready.png'), { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }], message1: b.execute(), args1: [{ type: 'input_statement', name: 'DO' }], colour: 265 },
    { type: 'bao_wait_image', message0: b.waitImage(), args0: [assetField(assets, 'button.png'), { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }, { type: 'field_number', name: 'TIMEOUT', value: 10000, min: 1, max: 3600000 }], previousStatement: null, nextStatement: null, colour: 205 },
    { type: 'bao_wait_image_state', message0: b.waitImageState(), args0: [assetField(assets, 'button.png'), { type: 'field_dropdown', name: 'STATE', options: [[appear, 'visible'], [disappear, 'hidden']] }, { type: 'field_number', name: 'TIMEOUT', value: 10000, min: 1, max: 3600000 }], previousStatement: null, nextStatement: null, colour: 205 },
    { type: 'bao_click_image', message0: b.clickImage(), args0: [assetField(assets, 'button.png'), { type: 'field_dropdown', name: 'BUTTON', options: [[b.leftButton(), 'left'], [b.rightButton(), 'right'], [b.middleButton(), 'middle']] }, { type: 'field_number', name: 'COUNT', value: 1, min: 1, max: 3 }, { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }, { type: 'field_checkbox', name: 'VERIFY', checked: true }, { type: 'field_number', name: 'MOVEMENT', value: 12, min: 0, max: 500 }], previousStatement: null, nextStatement: null, colour: 205 },
    { type: 'bao_move_to_image', message0: b.moveToImage(), args0: [assetField(assets, 'button.png'), { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }], previousStatement: null, nextStatement: null, colour: 205 },
    ...
    { type: 'bao_hold_key_until_image', message0: b.holdKeyUntilImage(), args0: [{ type: 'field_input', name: 'KEY', text: 'Space' }, assetField(assets, 'done.png'), { type: 'field_dropdown', name: 'STATE', options: [[appear, 'visible'], [disappear, 'hidden']] }, { type: 'field_number', name: 'TIMEOUT', value: 10000, min: 1, max: 3600000 }], previousStatement: null, nextStatement: null, colour: 120 },
    ...
    { type: 'bao_if_image', message0: b.ifImage(), args0: [{ type: 'field_dropdown', name: 'MODE', options: [[b.found(), 'found'], [b.notFound(), 'missing']] }, assetField(assets, 'button.png'), { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }], message1: b.then(), args1: [{ type: 'input_statement', name: 'THEN' }], message2: b.otherwise(), args2: [{ type: 'input_statement', name: 'ELSE' }], previousStatement: null, nextStatement: null, colour: 330 },
    ...
    { type: 'bao_condition_image', message0: b.imageCondition(), args0: [assetField(assets, 'button.png'), { type: 'field_number', name: 'THRESHOLD', value: .9, min: .1, max: 1, precision: .01 }], output: 'Boolean', colour: 205 },
    ...
    { type: 'bao_repeat_until_image', message0: b.repeatUntilImage(), args0: [assetField(assets, 'button.png'), { type: 'field_dropdown', name: 'UNTIL', options: [[appear, 'visible'], [disappear, 'hidden']] }, { type: 'field_number', name: 'MAX', value: 20, min: 1, max: 1000 }], message1: b.execute(), args1: [{ type: 'input_statement', name: 'DO' }], previousStatement: null, nextStatement: null, colour: 20 },
...
function compileBlock(LL: ReturnType<typeof useI18nContext>['LL'], block: Blockly.Block): AutomationStep {
  ...
  switch (block.type) {
    case 'bao_wait_image': return { ...extra, type: 'wait-image', asset: String(block.getFieldValue('ASSET')), threshold: number(block, 'THRESHOLD'), timeoutMs: number(block, 'TIMEOUT') } as AutomationStep;
    case 'bao_wait_image_state': return { ...extra, type: 'wait-image-state', asset: String(block.getFieldValue('ASSET')), state: block.getFieldValue('STATE') === 'hidden' ? 'hidden' : 'visible', timeoutMs: number(block, 'TIMEOUT') } as AutomationStep;
    case 'bao_click_image': return { ...extra, type: 'click-image', asset: String(block.getFieldValue('ASSET')), threshold: number(block, 'THRESHOLD'), button: block.getFieldValue('BUTTON'), clickCount: number(block, 'COUNT') as 1 | 2 | 3, verifyBeforeClick: block.getFieldValue('VERIFY') === 'TRUE', maxMovementPx: number(block, 'MOVEMENT') } as AutomationStep;
    case 'bao_move_to_image': return { ...extra, type: 'move-to-image', asset: String(block.getFieldValue('ASSET')), threshold: number(block, 'THRESHOLD') } as AutomationStep;
    ...
    case 'bao_hold_key_until_image': return { ...extra, type: 'key-hold-until-image', key: String(block.getFieldValue('KEY')), asset: String(block.getFieldValue('ASSET')), state: block.getFieldValue('STATE') === 'hidden' ? 'hidden' : 'visible', timeoutMs: number(block, 'TIMEOUT') } as AutomationStep;
    ...
    case 'bao_repeat_until_image': return { ...extra, type: 'repeat-until-image', condition: { ...(extra.type === 'repeat-until-image' ? extra.condition : {}), type: 'image-visible', asset: String(block.getFieldValue('ASSET')) }, until: block.getFieldValue('UNTIL') === 'hidden' ? 'hidden' : 'visible', maxIterations: number(block, 'MAX'), body: compileSequence(LL, block.getInputTargetBlock('DO'), extra.type === 'repeat-until-image' ? extra.body : undefined) } as AutomationStep;
...
function createStep(LL: ReturnType<typeof useI18nContext>['LL'], workspace: Blockly.WorkspaceSvg, step: AutomationStep): Blockly.BlockSvg {
  ...
  switch (step.type) {
    ...
    case 'click-image': setField(block, 'ASSET', step.asset); setField(block, 'THRESHOLD', step.threshold); setField(block, 'BUTTON', step.button); setField(block, 'COUNT', step.clickCount); setField(block, 'VERIFY', (step.verifyBeforeClick ?? false) ? 'TRUE' : 'FALSE'); setField(block, 'MOVEMENT', step.maxMovementPx ?? 12); break;
...
function createCondition(LL: ReturnType<typeof useI18nContext>['LL'], workspace: Blockly.WorkspaceSvg, condition: AutomationCondition): Blockly.BlockSvg {
  ...
  if (condition.type === 'image-visible') {
    setField(block, 'ASSET', condition.asset); setField(block, 'THRESHOLD', condition.threshold);
  ...
  useImperativeHandle(ref, () => ({
    compile: () => {
      ...
      return {
        ...
        ...(combined ? { readyWhen: compileCondition(LL, starts[0].getInputTargetBlock('CONDITION')) } : readyAsset ? { readyWhen: { ...readySource, type: 'image-visible' as const, asset: readyAsset, threshold: number(starts[0], 'THRESHOLD') } } : {}),
...
function loadIntoWorkspace(LL: ReturnType<typeof useI18nContext>['LL'], workspace: Blockly.WorkspaceSvg, workflow: AutomationWorkflow): void {
  ...
  if (workflow.readyWhen?.type === 'image-visible') { preserve(start, workflow.readyWhen); setField(start, 'ASSET', workflow.readyWhen.asset); setField(start, 'THRESHOLD', workflow.readyWhen.threshold ?? .9); }
