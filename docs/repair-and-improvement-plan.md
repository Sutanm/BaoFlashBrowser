...
> 执行状态（2026-08-02）：批次 1–2 已完成；批次 3 已修复 Ruffle 跨 session 资源协议、密码捕获开关、排除网站和关键生命周期问题，真实站点回归仍待完成；批次 4 已完成主进程任务账本、重启状态恢复、安全重试、进度节流与局部路径加固，模块拆分和真实引擎矩阵仍待完成；批次 5 已落地单元测试、BrowserView/Ruffle 冒烟测试和 CI，平台安装包仍待真实验证；批次 6 的会话恢复、本地 SWF 和诊断中心已完成；Toast 专项修复已完成。
