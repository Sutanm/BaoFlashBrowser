...
| 研究 | 文档 | 结论 |
|---|---|---|
| 颜色定位（多点找色）可行性 | `color-locating-poc-report.md` | 可用于候选召回和时序跟踪；同色异形会高分误报，不能单独作为默认点击依据 |
| 识图 Scale | `../superpowers/specs/2026-09-05-image-scale-notes.md` | 已落地主项目；新取材素材记录双端 transform，六场景 BrowserView/OpenCV 验收通过，待真实游戏手测 |
| 颜色/OpenCV 融合 | `../superpowers/specs/2026-09-05-color-matcher-aggregation-design.md` | 已落地“识色提案 + 结构复核”；现有鱼钩困难负样本通过，待扩充真实语料 |
...
2. `ImageLocator` 已支持显式 `method: 'template' | 'color' | 'auto'`；生产会话省略时进入带结构复核的自动路由，专业脚本仍可显式固定算法。
...
6. 跨页面 zoom 的 Surface scale 红色 REPRO 已修复：新 Surface 素材按 `logical比 × scaleX_mat/scaleX_run` 预测，旧/外部素材安全退回多尺度；双向 zoom 与双向窗口尺寸切换共六组 BrowserView/OpenCV 验收通过。
7. 颜色置信度校准保留为提案证据；最终接受增加固定结构分和独立候选差门禁，被拒绝候选不再显示误导性的 100%。
8. 当前自动路由源码矩阵是 15 个真实用例和 5 个生成用例；2026-09-06 复跑为路由 20/20、OpenCV 14/20、颜色 14/20。历史缓存中存在旧版 25 用例的 25/25，但已不对应当前源码。人物状态栏、树叶和收线局部等困难负样本尚未纳入，因此 `auto` 仍按实验能力管理。
9. `AutomaticVisionMatcher` 已接入 Runtime，并补齐取消、故障不降级、图片组分流和作者候选语义。
10. Runtime、助手和离线测试中心现已共享 OpenCV/颜色/自动路由实例；均先按真实阈值执行 Core 自动判定，失败后才单独获取 OpenCV 黄色诊断候选，诊断分数不会反向变成绿色成功。
11. OpenCV 与颜色 Worker 一起常驻预热；包注册时还会后台解码图片并预编译颜色签名。同一捕获帧的场景索引和像素传输可被图片组、回退倍率及重复查询复用。
12. 颜色 Worker 池按空闲状态领取完整素材，并使用一份只读共享帧；每个素材仍在同一 Worker 内完成全部倍率竞争，保持单图判定语义不变。真实 8 图全屏组由约 3.52s 压到约 0.90～1.45s，空闲机器可进入 1s，结果与逐图串行完全一致；鱼钩连续帧保持 86/86、误报 0/27。
13. 图片组取消改为资源屏障：调用方只有在全部并行 Worker 完成退出后才收到取消结果；门禁覆盖运行中取消、取消后立即再次识别和关闭竞态。
14. 外部素材首次成功后学习到的倍率现在先做单档快速首试，未命中再补 ±3% 邻档和普通/DPI 档。真实 8 图场景在 7 Worker 下，十档诊断搜索约 898ms，固定已知 1.5 倍的单档搜索约 283ms；该数字仅表示热态计算成本，召回安全仍由 miss-only 回退保证。
