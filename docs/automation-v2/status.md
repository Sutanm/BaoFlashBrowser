...
| 颜色/OpenCV 融合 | `../superpowers/specs/2026-09-05-color-matcher-aggregation-design.md` | 旧“坐标交集/颜色兜底”方案否决，待验证候选并集 + 结构复核 |
...
2. `ImageLocator` 已支持显式 `method: 'template' | 'color' | 'auto'`；生产会话省略时当前进入自动路由，但该默认行为仍待困难负样本与结构复核门禁，专业脚本可显式固定算法。
...
7. 颜色置信度校准在既有 0.75×/1×/1.25× 样本中通过，但真实页面已证明唯一性加成会产生误导性的 100% 和同色异形误报。
8. 当前自动路由源码矩阵是 15 个真实用例和 5 个生成用例；2026-09-06 复跑为路由 20/20、OpenCV 14/20、颜色 15/20。历史缓存中存在旧版 25 用例的 25/25，但已不对应当前源码。人物状态栏、树叶和收线局部等困难负样本尚未纳入，因此 `auto` 仍按实验能力管理。
...
11. OpenCV 与颜色 Worker 一起常驻预热；当前普通 Blockly/脚本省略算法时会进入 `auto`，但该默认行为需在结构复核门禁完成后重新评估。显式 `template` 保留稳定模板匹配入口。
